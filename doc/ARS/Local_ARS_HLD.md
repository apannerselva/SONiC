...
- After NHG is enabled through ARS, modifications to VLAN members - including additions, deletions, or MAC address movememnt will no longer initiate ARS re-selection
...
2. ARS_GROUP table used to specify ARS nexthop group configuration parameters. ArsOrch uses this to create the L3 ARS SAI object.

3. ARS_INTERFACES holds list of L2 interfaces which are ARS-enabled. NHG will be considered ARS-enabled when all of its associated nexthop interfaces are present in the configured ARS_INTERFACES when the match mode configured in ARS_PROFILE is interface-based.

3. ARS_NEXTHOPS holds list of nexthops which are used for identifying ARS-enabled nexthop group. NHG will be considered ARS-enabled when all of its associated nexthops are present in the configured ARS_NEXTHOP_MEMBER entries when the match mode configured in ARS_PROFILE is nexthop-based and additionally its interface should be L2 ARS-enabled interfaces. 

6. ARS_PORTCHANNELS table hold list of lag interfaces associated with ARS_PORTCHANNEL_GROUP objects.

7. An NHG can be enabled either through the ARS_INTERFACES or the ARS_NEXTHOPS_GROUP, both of which are user-configurable.
...
The ars_nhg_path_selector_mode field in ARS_PROFILE defines the scope at which path selection for ARS nexthop groups is applied.  
The system supports three distinct modes:

**1. Global**:
In global mode, path selection is managed at a system-wide level.Only ARS_PROFILE and ARS_GROUP need to be explicitly configured.  
ARS_INTERFACES are automatically created by the software and require no manual configuration. After creating an ARS_GROUP, the user is responsible for assigning it to the default_ars_group  
field in the corresponding ARS_PROFILE.

**2. Interface**:
In interface mode, path selection is applied per interface. Users must explicitly create ARS_INTERFACES, in addition to ARS_PROFILE and ARS_GROUP.  
Each ARS_INTERFACE must be associated with the appropriate ARS_GROUP, allowing different interfaces to use different path selection logic.

**3. Nexthop**:
In nexthop mode, path selection is applied at the individual nexthop level.  
Configuration requires the creation of ARS_PROFILE, ARS_GROUP, ARS_INTERFACES and ARS_NEXTHOPS.  

This mode provides the most granular level of control, enabling ARS over ARS nexthop members.


**ARS selection for LAG**
The ars_lag_path_selector_mode field in ARS_PROFILE defines how path selection is applied for  
ARS LAG configurations. This setting controls whether LAG path selection is handled globally or per interface.  
The system supports the following two modes:

**1. Global**:
In global mode, LAG path selection is managed at a system-wide level. Only ARS_PROFILE and ARS_GROUP need to be explicitly configured.  
ARS_INTERFACES are created implicitly by the software. After creating an ARS_GROUP, the user must assign it to the default_ars_group field in the associated ARS_PROFILE.  

This mode simplifies configuration by applying a uniform LAG selection policy across all interfaces.

**2. Interface**:
In interface mode, LAG path selection is applied on a per-interface basis. The user is responsible for explicitly creating ARS_INTERFACES, in addition to ARS_PROFILE and ARS_LAG_GROUP.  
Each ARS_INTERFACE must be associated with a specific ARS_GROUP to define interface-specific LAG path selection behavior.  

This mode provides more granular control, allowing different interfaces to utilize different LAG selection policies.


**ARS Path Selection Criteria**

ARS functionality will only be enabled if the following criteria are met:

**1. Global**:
In Global mode, the user must configure the default_ars_group parameter. If this parameter is not set, ARS path selection will not occur.

**2. Interface**:
In Interface mode, ARS group selection follows a hierarchical lookup:  
First, the system checks for a value in the ars_grp_name field of the ARS_INTERFACES table.  
If no ARS group is defined in ARS_INTERFACES, the system will fall back to the default_ars_group defined in ARS_PROFILE.  
It is the user’s responsibility to ensure that all interfaces associated with a NHG or LAG are mapped to the same ARS group.  
This is necessary for ARS capabilities to be correctly applied to the NHG or LAG.

**3. Nexthop**:
In Nexthop mode, ARS group selection also follows a defined hierarchical lookup process:  
The system first attempts to retrieve the ARS group from the ars_grp_name field in the ARS_NEXTHOP_MEMBER table.  
If no ARS group is specified in ARS_NEXTHOP_MEMBER, the system will default to the default_ars_group value defined in the ARS_PROFILE.  
This hierarchical selection mechanism is essential to enable ARS capabilities for the corresponding Next Hop Group (NHG).  

Additionally, users are required to create appropriate entries in the ARS_INTERFACE table for interfaces involved in the NHG.  
Failure to define ARS_INTERFACE mappings will result in those NHG being excluded from ARS-capable NHG behavior.


**Important Consideration**:

If the user does not define corresponding entries in the ARS_INTERFACE table for ports that are part of a NHG or LAG,  
those ports will not be considered ARS-capable.  
It is the user’s responsibility to ensure that all interfaces participating in a NHG or LAG are:

- Properly configured in the ARS_INTERFACE table.
- Mapped to the same ARS group for ARS functionality to be applied consistently.

Once an NHG or LAG is formed, users must not:
- Remove the associated interfaces from the ARS_INTERFACE table.
- Modify the ARS_GROUP associated with those interfaces.

Any such changes made after NHG or LAG formation will not reflect in ARS behavior, and the new configuration will not be applied.
...
4. Add ARS_GROUP Table
    * The CONFIG_DB adds the ARS_GROUP table, which defines the ARS specific nexthop configuration parameters. ArsOrch uses this to create the ARS SAI object (logically a L3 ARS SAI object).

5. ARS_NEXTHOPS Table
    * The CONFIG_DB adds the ARS_NEXTHOPS table, which specifies which nexthops are used for identifying ARS nexthop group. If all of nexthops in added NHG, exist in ARS_NEXTHOPSand are pointing to ARS-enabled L2 interface, that NHG is eligible for ARS.

6. Add ARS_INTERFACES Table
    * The CONFIG_DB adds the ARS_INTERFACES table, causing ArsOrch to enable ARS on the corresponding ports and also used for identifying ARS nexthop group.
  
7. Add ARS_PORTCHANNELS Table
    * The CONFIG_DB adds the ARS_PORTCHANNELS table, to associate LAG to alternative_path.
...
2. Notification
    ...
    * ArsOrch will verify that received notification matches the nexthops in ARS_NEXTHOPS and that nexthops are defined over ARS-enabled ports
3. ars_nhg_path_selector_mode
    * ARS will determine whether ars_nhg_path_selector_mode is global-based, interface-based or nexthop-based.
        - In global based mode, default ars group will be matched for all the interfaces.
...
1. Add ARS_GROUP Table
    * The CONFIG_DB adds the ARS_GROUP table, triggering the process.
2. ars_lag_path_selector_mode
    * ARS will determine whether ars_lag_path_selector_mode is global-based or interface-based.
        - In global based mode, default ars group will be matched for all the interfaces.
        - In interface-based mode, only the interfaces will be matched.
2. Add ARS_PORTCHANNELS Table 
    * The CONFIG_DB adds to associate LAG to alternative_path.
    ...
    container ARS_PROFILE {
        ...
        list ARS_PROFILE_LIST {
            ...
            leaf ars_nhg_path_selector_mode{
                 description "Path selection mode for nexthop (global/interface/nexthop)";
                type enumeration {
                    enum global {
                        description "ARS Nexthop group selection is global.";
                    }
                    enum interface {
                        description "Uses list of configured interfaces for ARS nexthop group selection.";
                    }
                    enum nexthop {
                        description "Uses list of configured nexthops for ARS nexthop group selection.";
                    }
                } 
                default "interface";
            }
            ...
            leaf ars_lag_path_selector_mode {
                description "Path selection mode for LAG (global/interface)";
                type enumeration {
                    enum global {
            ...
            leaf default_ars_group {
                description "Default ARS group name for both nexthop group and LAG.";
                type leafref {
                    path "/sars:sonic-ars/sars:ARS_GROUP/sars:ARS_GROUP_LIST/sars:default_ars_group";
                }
            }
...
```
    container ARS_GROUP {
        ...
        description "ARS-enabled Groups";
        ...
        /* end of list ARS_GROUP_LIST */
    }
    /* end of container ARS_GROUP */
```
...
```
    container ARS_NEXTHOPS{
        ...
        list ARS_NEXTHOP_LIST {
            ...
            leaf ars_grp_name {
                description "ARS Group name, inherited from 'default_ars_group' unless explicitly configured";
                type leafref {
                    path "/sars:sonic-ars/sars:ARS_GROUP/sars:ARS_GROUP_LIST/sars:grp_name";
                }
            }
            leaf role {
                type enumeration {
    ...
    container ARS_INTERFACES {
        ...
        list ARS_INTERFACE_LIST {
            description  "List of configured interfaces participating in ARS and
                          used for ARS Nexthop Group selection when ars_nhg_path_selector_mode
                          as interface/nexthop and also used for ARS LAG selection
                          when ars_lag_path_selector_mode as interface”
            key "if_name";
...
ars_nhg_path_selector_mode       = "global"/     ; ARS nexthop path selector mode, to select the NHG for ARS enabled NHG
                                   "interface"/  ; - 'global'    : ARS selection is enabled on all interfaces by default.
                                   "nexthop"     ; - 'interface' : ARS selection is based on interface list  
                                                 ; - 'nexthop'   : ARS selection is based on nexthop list
                                                 ; Default : "interface"
ars_lag_path_selector_mode       = "global"/     ; ARS nexthop path selector mode, to select the NHG for ARS enabled NHG
                                   "interface"/  ; - 'global'    : ARS selection is enabled on all interfaces by default.
                                   "nexthop"     ; - 'interface' : ARS selection is based on interface list  
                                                 ; - 'nexthop'   : ARS selection is based on nexthop list
                                                 ; Default : "interface"
default_ars_group                = string        ; Default ARS group name (optional)
...
"ARS_PROFILE": {
    "ars_profile_default": {
        ...
        "default_ars_group" : "ars_grp_name",
        "max_flows" : "512",
...
```
; New table ARS_GROUP_TABLE
; Nexhop groups enabled for ARS

key                       = ARS_GROUP|grp_name                        ;Name which identifies ARS-nexhop-group 
...
```
; New table ARS_NEXTHOPS_TABLE
; Nexthop IPs associated with ARS-enabled Nexthop group

key                      = ARS_NEXTHOPS|vrf_name|nexthop_ip            ;Nextop IP identifing nexhop-group member 

;field                   = value

ars_grp_name             = string                                            ;ARS nexthop group Name
...
```
```
; New table ARS_INTERFACES_TABLE
...
```
; New table ARS_PORTCHANNELS_TABLE
...
5. ARS for Nexthop Groups (L3 traffic)
    ...
    * Validate when ars_nhg_path_selector_mode is interface mode.
        ...
        * Validate when ars_nhg_path_selector_mode is Nexthop.
    * Validate when ars_nhg_path_selector_mode is global and nexthop mode.
...
8. ARS over LAGs (L2 traffic, Phase 2)
    ...
    * Validate when ars_lag_path_selector_mode is interface mode.
        ...
        * Validate per flowlet/per packet distribution.
    * Validate when ars_nhg_path_selector_mode is global mode.
        *  Repeat all validation steps listed above.
